...
const findUserByEmail = async (email: string): Promise<User | null> => {
  ...
  if (user) return { ...user, role: "SECRETARIO" } as User;

  user = await prisma.professor.findUnique({ where: { email } });
  if (user) return { ...user, role: "PROFESSOR" } as User;

  user = await prisma.aluno.findUnique({ where: { email } });
  if (user) return { ...user, role: "ALUNO" } as User;
...
export const login = async (email: string, password: string) => {
  const JWT_SECRET = JWT_SECRET_RAW?.trim();
  if (!JWT_SECRET)
